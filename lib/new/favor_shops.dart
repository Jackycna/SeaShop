...
class FavShops extends StatefulWidget {
  const FavShops({super.key});

  @override
  OwnerDetailsPageState createState() => OwnerDetailsPageState();
}

class OwnerDetailsPageState extends State<FavShops>
    ...
    if (userDoc.exists) {
      ...
      if (userData != null) {
        if (mounted) {
          setState(() {
            // Fetch user location
            _userLocationString = userData.containsKey('address')
                ? userData['address'] // Stored as "lat,lng"
                : null;

            // Fetch favorite shops (returns null if not found)
            favoriteShopIds = userData.containsKey('favoriteShops')
                ? List<String>.from(userData['favoriteShops'])
                : null;
          });
        }
      }
    ...
    return _firestore
      ...
      for (var doc in snapshot.docs) {
        ...
        if (ownerData != null && ownerData.containsKey('shopLocation')) {
          ...
          // 🔹 Only add shop if it's in the favorite list
          if (favoriteShopIds != null && favoriteShopIds!.contains(doc.id)) {
            shops.add({
              "id": doc.id,
              "data": ownerData,
              "distance": distance,
          ...
          : StreamBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                if (!snapshot.hasData || snapshot.data!.isEmpty) {
                  return const Center(child: Text('No FavShops available.'));
                }
